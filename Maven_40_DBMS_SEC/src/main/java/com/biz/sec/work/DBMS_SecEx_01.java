 ...
 * 평문문자열(plainText): 읽을 수 있는 문자열
 * 암호화된 문자열(cryptText): 읽을수는 있지만 무슨 내용인지 알 수 없는 형태
 */
...
public class DBMS_SecEx_01 {
	public static void main(String[] args) {
		 ...
		 * 				보안은 유리하지만, 사용에 제약이 따른다.
		 * 		나. 양방향 암화화
		 * 				평문 >> 암호문, 암호문 >> 평문 변경이 가능
		 * 				보안성은 다소 떨어지지만 사용에 다소 편리성이 있다.
		 ...
		 *  
		 *  최소한 소스코드차원에서만이라도 userid와 password를 암호화하여 보안을 유지하기 위한 방법을 사용환다.
		 *  그런데 이방법으로 단방향 암호화 기법은 사용이 불가능하다.
		 *  jdbc에서 DBMS로 userid와 password를 전송하는 과정에서 평문으로 해당 데이터를 보내야 하기 때문에
		 *  암호화된 문자열을 다시 평문으로 복호화해야 한다.
		 *  이런 용도로 주로 사용하는 도구가 jasypt이고 그 중에서 StandardPBEStringEnryptor를 많이 사용한다.
		 */
...
//		username : ToXXyJ0kpRDo1Hzhlgp5Rw==
//		password : glEcG0oOGlGAM+euGXLpBA==
